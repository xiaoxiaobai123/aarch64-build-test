...
def get_company_bar(width):
    ...
    if bar is None:
        ...
        cv2.putText(bar, "TEST MODE - NO HALCON",
...
def make_fake_camera_image(cam_num, tick, result, width=1024, height=1280):
    ...
    # 画文字标识
    cv2.putText(img, f"Cam {cam_num}", (50, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 255, 255), 4)
    cv2.putText(img, f"Tick {tick:04d}", (50, 170),
                cv2.FONT_HERSHEY_SIMPLEX, 1.8, (255, 255, 255), 3)
    cv2.putText(img, result.name, (50, height - 50),
                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 255, 0), 4)

    # 十字准星
    cv2.line(img, (center[0] - 30, center[1]), (center[0] + 30, center[1]),
             (255, 255, 255), 2)
    cv2.line(img, (center[0], center[1] - 30), (center[0], center[1] + 30),
             (255, 255, 255), 2)
...
def main():
    parser = argparse.ArgumentParser(
        description='显示链路测试 (无 halcon 依赖)',
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--path', default='/home/pi/output_image.rgb565',
                        help='rgb565 输出文件路径')
    parser.add_argument('--interval', type=float, default=1.0,
                        help='每帧间隔秒数')
    parser.add_argument('--count', type=int, default=0,
                        help='跑 N 帧后退出, 0 = 无限循环')
    parser.add_argument('--size', default='1024x1280',
                        help='单相机图尺寸, 格式 WxH')
    parser.add_argument('--profile', action='store_true',
                        help='每帧打印各环节耗时细分, 方便定位瓶颈')
    ...
    if not output_path.exists() and not output_path.is_symlink():
        ...
        print(f"    建议先跑 deploy/install.sh 建立软链接和 tmpfs 文件")
        print()
    ...
    try:
        while args.count == 0 or tick < args.count:
            ...
            if args.profile:
                print(f"         fake={(t_fake-t0)*1000:5.1f}ms  "
